...
3.  **Visual Odometry**: By aligning the *current* minimap with the *recorded* minimap, we can extract error vectors that map directly to controls.
...
-   **Heading Error (`d_theta`)**: The rotation difference between current and target minimap.
    -   *Action*: Press Camera Left/Right (4/6).
-   **Lateral Error (`dx`)**: The horizontal shift.
    -   *Action*: Press Strafe Left/Right (A/D).
-   **Longitudinal Error (`dy`)**: The vertical shift.
    -   *Action*: Press Forward/Back (W/S).
...
-   **Drift Monitor (Debug Tool)**:
    ...
    -   *Validation*: Verify that rotating the camera generates a pure `d_theta` signal.
